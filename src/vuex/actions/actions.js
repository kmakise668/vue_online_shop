...
export default {
    GET_PRODUCTS_FROM_API({ commit }) {
        return axios
            .get('http://localhost:7777/api/products') // Используйте метод get для получения данных
            .then((response) => {
    ...
    async addProduct({ commit, dispatch }, productData) {
        try {
            const response = await axios.post('http://localhost:7777/api/products', productData);
